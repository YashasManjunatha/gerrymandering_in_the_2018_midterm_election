...
PrecPATH = PPATH+'Data/ShapeFiles/Precincts'
FPATH = PPATH+'Data/ExtractedData'
VOTEPATH = PPATH+'Data/ElectionData/Extracted'
...
for LAYER_NAME in LAYER_NAMES:
    ...
    for layerQ in layers:
        if layerQ.name() == LAYER_NAME:
            layerLoaded = True
    if not layerLoaded:
        shapeFilePath = IPATH+SLASH+LAYER_NAME+SLASH+LAYER_NAME+"C.shp"
        qgis.utils.iface.addVectorLayer(shapeFilePath,LAYER_NAME,"ogr")
        # refresh the layers list
        layers = qgis.utils.iface.legendInterface().layers()

wholeStatePrecinctShapeFiles = glob(PrecPATH+"/SBE_PRECINTS*/*.shp")
print wholeStatePrecinctShapeFiles

for year in YEARS:
    for wholeStatePrecinctShapeFile in wholeStatePrecinctShapeFiles:
        if str(year) in wholeStatePrecinctShapeFile:
            shapeFileName = wholeStatePrecinctShapeFile
    print year, shapeFileName
    #load in precinct file by year
