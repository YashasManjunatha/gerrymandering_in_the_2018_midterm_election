...
for state in state_codes:
	...
	for tract_feat in tractLayer.getFeatures():
		...
		print(type(centroid.x()), type(centroid.y()))
		distances = []
		for c in currCentroids:
			distances.append(centroid.distance(currCentroids[c]))
...
with open(impactRepoPath + "tract_municipality_distance.csv", mode = "w") as output:
